...
`hyperlightweight-ai`의 독자 학습 경로입니다. 이 경로는 외부 사전학습 가중치, 외부 토크나이저, 모델별 채팅 템플릿, Hugging Face 모델 클래스를 사용하지 않습니다. 모델은 빈 가중치에서 시작하며, 저장소 안의 순수 PyTorch 구현으로 학습과 추론을 수행합니다.
...
바이트 단위 토큰화는 별도의 언어별 vocabulary 파일 없이 한국어, 영어, 이모지, 소스 코드를 모두 표현할 수 있다는 장점이 있습니다. 반대로 한국어 한 글자가 여러 바이트가 되므로 같은 문맥 길이에서 처리할 수 있는 의미 단위가 줄어드는 단점도 있습니다. 이 선택은 독립성과 재현성을 우선한 것입니다.
